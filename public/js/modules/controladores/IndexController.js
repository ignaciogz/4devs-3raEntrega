...
class IndexController {
    static async ejecutar() {
        ...
        let viewData = Object.assign(dataProducts, dataCart, dataAuth);
        ...
        IndexController.addEventHandlers(dataAuth.isLogged);
    }

    static addEventHandlers(isLogged) {
        if(isLogged) {
            ...
            for (const addBtn of addBtns) {
                addBtn.addEventListener("click", function (e) {
                    ...
                    res.then(data => {
                        const $items = document.getElementById("itemsNumber");
                        $items.textContent = Number($items.textContent) + 1;
                    });
                });
            }
            ...
            formEditProduct && formEditProduct.addEventListener("submit", e => {
                e.preventDefault();
